...
const App: React.FC = () => {
  ...
  const [appConfig, setAppConfig] = useState<AppConfig>(() => {
    const savedConfig = localStorage.getItem('appConfig_v3'); // Version bumped for payment config
    if (savedConfig) {
      try {
    ...
    return {
      ...
      payment: {
        accessToken: 'APP_USR-3981464870256972-092413-3cac81a1b58892ee959b900e6998aa81-2081587558', // Default from prompt
  ...
  const handleLoadingComplete = () => {
    // Step 2: Animation finished, show Plan Selection
    setIsLoading(false);
    setShowPlanModal(true);
  };

  const handlePlanSelect = (plan: 'basic' | 'complete') => {
    // Step 3: Plan selected, show Registration
  ...
  const handleAdminSave = (newConfig: AppConfig) => {
    ...
    localStorage.setItem('appConfig_v3', JSON.stringify(newConfig));
  };
    ...
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 shadow-2xl relative overflow-hidden font-sans text-gray-800">
      ...
      <nav className="fixed bottom-0 max-w-md w-full bg-white border-t border-gray-100 px-6 py-3 flex justify-between items-center z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <button 
          ...
          className={`flex flex-col items-center gap-1 ${currentView === AppView.HOME ? 'text-brand-600' : 'text-gray-400'}`}
        >
